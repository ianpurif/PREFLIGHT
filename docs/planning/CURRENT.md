...
- On 2026-09-12 the official Speculos 0.27.0/Nano S Plus 1.22.3 session was reproduced from the current checkout. DMK returned the public test address, while the exact signing attempt failed closed before `signTypedData` with `CLEAR_SIGNING_UNAVAILABLE` because the Ledger-issued origin token is absent. No signature or authorization was produced; see `docs/compliance/evidence/p20-ledger-speculos-authorization-blocked-2026-09-12.md`.
