...
4. **Deterministic evaluator** validates exact declared build/envelope/evaluator bindings, generates the committed suite, checks materialized trace metadata/coverage, and emits an internal report. P3's future CRE confidential handler must authenticate trace provenance and invoke this same pure core inside the TEE.
...
Canonical schema/digest rules are defined in ADR-0003 and deterministic evaluation semantics in ADR-0004. No step may silently downgrade to trusting a frontend boolean or an uncommitted rule/scenario set.
