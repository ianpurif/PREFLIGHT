...
5. **Confidential CRE input** is one versioned secret containing the full private envelope and 32-byte blind. It is fetched only by the Nitro `handlerInTee` callback using the request's site-bound selector; the legacy fixed selector is accepted only by old simulation payloads that omit the selector. When configured, the handler may use the official HTTP capability to deliver the already-minimal public result to the API; it never sends confidential values.
